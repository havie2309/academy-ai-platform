...
| Bảo mật | D-14 · Validate upload security level theo role: chặn user upload tài liệu vượt quá security level được phép | M2 | `[x]` | `documents.service.ts` + `documents.controller.ts` enforce `maxSecurityLevel` and `SECURITY_RANK` check; UI `DocsPage.tsx` filters dropdown options based on user's `maxSecurityLevel`. |
...
| AI      | E-10 · Nâng cấp AI Policy từ keyword-based lên semantic/context-based | M3 | `[x]` | `safe_refusal.py` calls `evaluate_guardrails()` → `match_semantic()`; `pipeline.py` conditionally invokes `match_semantic` based on `GUARDRAIL_SEMANTIC_ENABLED`; `admin-config` supports `matchMode: 'semantic'` and threshold; guardrail rules loaded from policy store. |
...
| Bảo mật | G-13 · Thêm format validation cho access token trước khi truy vấn DB/memory | M5 | `[x]` | `jwt-token-format.ts` exports `isWellFormedJwtCompactToken()` and `extractFormattedBearerJwt()`; `gateway-auth.ts` uses these to reject malformed tokens before DB/Redis checks. |
...
| AI | J-02 · Hỗ trợ bài tập, giải thích từng bước | M6 | `[x]` | `exercises.py` + `documents.service.ts` + `DocsPage.tsx`: uses parent chunks, MongoDB cache (`document_exercises`), background generation with distributed lock (`exercise_jobs`), separate LLM config (`EXERCISE_*`), polling-based status, DOCX export. |
...
| UI | K-13 · Admin Chat Monitoring: xem chat history/session của các user khác | M6 | `[x]` | `chat.controller.ts` + `chat.service.ts` expose `GET /admin/sessions` and `GET /admin/sessions/:sessionId/messages`; `AdminPage.tsx` has chat monitor modal with pagination and session/message view. |
...
| UI | K-15 · Admin Scope Management: gán/sửa vùng dữ liệu (access scope) cho tài liệu và tài khoản | M6 | `[x]` | `documents.controller.ts` + `documents.service.ts` expose `PATCH /:id/scope`; `DocsPage.tsx` has scope edit modal with pre-filled fields, calling `docsApi.updateScope()`. |
| UI | K-16 · Admin Dashboard UX: chuyển sang tab view, pagination, giới hạn hiển thị | M6 | `[x]` | `AdminPage.tsx` uses `ADMIN_TABS` and `activeTab` state; `AdminOpsSection.tsx` implements pagination (`page`, `total`, `PAGE_SIZE`); `AdminAuditSection.tsx` has limit filter (25/50/100/200). |
