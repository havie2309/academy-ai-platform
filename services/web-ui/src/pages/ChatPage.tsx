...
import { BookOpen, Database, Sparkles, FileText, Calendar, ArrowUp } from 'lucide-react'
...
function generateMockCitations(query: string): Citation[] {
  ...
  } else if (q.includes('điểm') || q.includes('gpa') || q.includes('học lực') || q.includes('k65') || q.includes('trung bình')) {
    return [
  ...
  } else if (q.includes('tài liệu') || q.includes('học máy') || q.includes('môn') || q.includes('giáo trình')) {
    return [
  ...
  return [
    { id: 'c1', title: 'Cơ sở dữ liệu tri thức tích hợp EduMind', source: 'Hệ thống Học viện' }
  ]
}
  ...
  const send = async (text?: string) => {
    ...
    try {
      ...
      const botMsg: Message = {
        ...
        role: 'assistant',
        content: res.answer,
        citations: (res.citations && res.citations.length > 0)
    ...
    <div className="flex flex-col h-full bg-slate-50/50">
      ...
      <div className="h-15 border-b border-slate-200/60 bg-white/85 backdrop-blur-md flex items-center justify-between px-6 shrink-0 z-10 shadow-[0_1px_3px_rgba(0,0,0,0.01)]">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-bold text-slate-700">Smart Assistant</span>
        </div>
      </div>
          ...
          <div className="flex flex-col items-center justify-center min-h-full gap-8 px-6 py-12 relative overflow-hidden">
            ...
            <div 
              ...
              style={{
                top: '45%',
                left: '50%',
                background: 'radial-gradient(circle, rgba(59, 130, 246, 0.38) 0%, rgba(147, 197, 253, 0.16) 50%, rgba(255, 255, 255, 0) 80%)',
                filter: 'blur(75px)',
              }}
            />
            ...
            <div className="text-center max-w-lg relative z-10">
              ...
              <p className="text-slate-500 mt-2 text-sm md:text-base leading-relaxed font-medium">
                Hệ thống trợ lý học tập thông minh. Tôi có thể giúp gì cho bạn hôm nay?
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3.5 w-full max-w-3xl mt-4 relative z-10">
              {suggestions.map(({ icon: Icon, text, category }) => (
                <div
                  ...
                  tabIndex={0}
                  onClick={() => send(text)}
                  onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') send(text) }}
          ...
          <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
            {messages.map((msg) => (
              ...
              >
                ...
                {msg.role === 'user' ? (
                  <div
                    className="rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap shadow-sm bg-blue-600 text-white rounded-tr-none font-medium max-w-[80%]"
                  >
                    {msg.content}
                  </div>
                ) : (
                  <div className="flex flex-col max-w-[80%] gap-2 flex-1 min-w-0">
                    <div
                      className="rounded-2xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap shadow-sm bg-white text-slate-800 border border-slate-200/50 rounded-tl-none"
                    >
                      {msg.content}
                    </div>
                      ...
                      <div className="mt-1 px-1">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {msg.citations.map((cit, idx) => (
                            <div
                              key={cit.id || idx}
                              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-50/50 hover:bg-blue-50 border border-blue-100/60 text-xs text-blue-700 hover:text-blue-800 font-medium transition-all shadow-[0_1px_3px_rgba(0,0,0,0.01)]"
                              title={cit.title}
                            >
                              <FileText size={12} className="text-blue-500 shrink-0" />
                              <span className="truncate max-w-[180px] md:max-w-[240px]">{cit.title}</span>
                              <span className="text-[10px] bg-blue-100/70 px-1 py-0.5 rounded text-blue-600 shrink-0">{cit.source}</span>
                            </div>
                          ))}
                        </div>
                      </div>
            ...
            {loading && (
              <div className="flex gap-4 justify-start">
                ...
                <div className="flex items-center gap-1.5 px-4 py-3 bg-white border border-slate-200/50 rounded-2xl rounded-tl-none shadow-sm">
                  <span className="w-2.5 h-2.5 bg-blue-400 rounded-full animate-bounce [animation-delay:0ms]"></span>
                  <span className="w-2.5 h-2.5 bg-blue-400 rounded-full animate-bounce [animation-delay:150ms]"></span>
                  <span className="w-2.5 h-2.5 bg-blue-400 rounded-full animate-bounce [animation-delay:300ms]"></span>
                </div>
              </div>
            )}
            <div ref={bottomRef} />
      ...
      <div className="shrink-0 px-4 pb-6 pt-2">
        <div className="mx-auto max-w-3xl">
          ...
          <p className="text-center text-[11px] text-slate-400 mt-2 font-medium">
            Smart assistant có thể đưa ra câu trả lời chưa chính xác. Vui lòng kiểm chứng thông tin quan trọng.
          </p>
        </div>
      </div>
    </div>
