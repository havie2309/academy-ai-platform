...
| `GET` | `/api/documents/:id/chunks` | JWT | `id` trên path, query `limit` (mặc định 5, tối đa 1000), `chunkType?` (`child` or `parent`, default `child`), `chunk_ids?` (array) | `{ chunks: [{ id, text, index, section_path, page, created_at }], total }` | Không đủ quyền -> `403`; không tìm thấy -> `404` | `implemented` |
