...
| Dữ liệu | C-06 · Seed dữ liệu nghiệp vụ mẫu và user đa role               | M0  | `[-]`   | IAM **partial done**: `generate_seed.py` + `12-seed-iam.sql` đã chuyển sang `pbkdf2_sha256` + `password_salt` + `hash_iterations=100000`, đồng bộ lại các seed `11–14`; `seed-iam.ps1`; **chưa** validate full re-run generator / seed fresh DB end-to-end |
...
| AI      | D-06 · Hybrid chunking, overlap, giữ cấu trúc           | M2  | `[x]`   | Đã hoàn thiện parent‑child chunking. **Quyết định kiến trúc:** nhánh line-based/tabular split từ v1 đã được loại bỏ (không cần thiết vì `extract.py` đã chuẩn hóa dữ liệu qua MarkItDown trước khi chunk). Parent chỉ tạo tại `Điều/Mục` (không flush `Phần/Chương`). Đã sửa bug chapter header bị flush thành parent. Bổ sung unit test `tests/test_chunker.py` (6 tests) và `tests/test_extract.py` (5 tests) làm regression baseline cho implementation hiện tại. Evidence: `python -m unittest tests.test_chunker -v` và `python -m unittest tests.test_extract -v` đều pass. (2026‑06‑24) |
| AI      | D-07 · Prefix ngữ cảnh trước khi embed                  | M2  | `[x]`   | Đã inject metadata vào child text trước embed: `[Văn bản: title] -> [Vị trí: section_path] -> [Nội dung]: ...` trong `pipeline.py`. Nghiệm thu: kiểm tra log embedding request / retrieval output thấy prefix xuất hiện đúng. (2026‑06‑24) |
| AI      | D-08 · Batch embed, upsert Milvus, sync catalog MongoDB | M2  | `[-]`   | `services/document-processor/app/pipeline.py` nay batch embed trên `child_nodes`, lưu cả `parent` + `child` vào `document_chunks`, insert vector child vào Milvus và backfill `milvusVectorId`; vẫn có guard lệch số vector. **Còn** cần rà thêm cleanup/sync parent-child sau re-ingest để nghiệm thu lại end-to-end |
...
| AI      | E-02 · Query embed, Milvus top-k, Mongo fetch chunk   | M3  | `[-]`   | `rag-engine/app/retrieval.py` đã embed query qua `embedding-server`, search **child chunks** trong Milvus, fetch child docs từ Mongo rồi suy ra `parentId` để lấy **parent chunks** làm citation; phụ thuộc parent-child persistence path nên vẫn cần verify thêm |
...
| AI      | E-04 · Rerank cross-encoder và chọn context           | M3  | `[x]`   | Đã nâng cấp reranker lên `jinaai/jina-reranker-v2-base-multilingual` (cross‑encoder mạnh, đa ngôn ngữ). Flow: Milvus top‑K (15) → rerank → chọn top `RERANK_TOP_K` (5). Nghiệm thu: gọi `/v1/retrieve` trả về 200 OK; `/v1/chat/stream` hoạt động bình thường. (2026‑06‑24) |
...
| Backend | G-07 · Rate limit và throttling theo role (gateway)          | M5  | `[x]`   | Đã triển khai rate limiting dựa trên Redis với mức giới hạn khác cho authenticated (`RATE_LIMIT_AUTH=60/min`) và anonymous (`RATE_LIMIT_ANON=10/min`). Tích hợp vào `api-gateway` như middleware. (Commit `eeafe5bf`) |
...
| UI   | K-07 · Admin audit viewer và export                        | M6  | `[-]`   | `services/platform/apps/audit/src/audit.service.ts` đã hỗ trợ filter `status/action/resourceType/userId/resourceId/from/to` và cap limit; `services/web-ui/src/api/admin.ts` đã có `getAuditLogs/getAuditLog`; **còn thiếu:** `services/web-ui/src/pages/AdminPage.tsx` chưa render viewer/detail/export, Playwright `admin.spec.ts` hiện mới cover health + policy |
...
| AI        | Nhóm Trợ lý ảo nâng cao | M6  | J, K       | `[-]`   | Phase 1 chat+history `[x]` (G-12, K-12); Phase 2 **gần xong:** K-03 citation/SSE + SQL table render, ingest/index pipeline (**D phần lớn `[x]`, parent-child chunk/retrieval đang polish**), RAG+SQL+safe refusal (E/F `[x]` phần lớn), admin health/policy (K-08/K-09 `[x]`), Playwright smoke (K-11 `[x]`); **chưa:** J-01..J-06 nâng cao |
...
**Trạng thái chat hiện tại** — Nền chat/RAG + SQL read-only + policy/RBAC an toàn đã nối end-to-end qua gateway; ingest/OCR/chunk/embed v1 đã có, nhưng nhánh **parent-child chunking / retrieval** mới pull về vẫn đang polish ở **D-06..D-08 / E-02**. Phần còn thiếu rõ nhất: **K-10** (quota/token/account ops), **K-04** phase 2 (timeline/chunk preview), **E-01** (reject/task-assist) và **C-03** (Milvus metadata push-down).
...
| 11 | **K-07** | Hoàn thiện audit panel trong `/admin`: list/filter/detail/export; backend filter + web-ui API helper đã có một phần |
...
| 17 | **D-06..D-08 / E-02** | Polish parent-child chunking + retrieval, verify lại sync/re-ingest/citation end-to-end |
...
- **K-07 hiện ở mức partial** — audit backend đã có filter/query helper và `web-ui` đã có wrapper gọi `/api/audit/logs`, nhưng `AdminPage` hiện chưa render viewer/detail/export; Playwright `admin.spec.ts` hiện mới cover health + policy |
...
- **D-06..D-08 + E-02 làm một phần** (2026-06-23) — `document-processor` + `rag-engine` bắt đầu chuyển sang parent-child chunking/retrieval: search child chunks trong Milvus rồi trả parent citation từ Mongo; phần persistence/sync/re-ingest và prefix ngữ cảnh trước embed vẫn cần verify thêm trước khi nghiệm thu lại |
