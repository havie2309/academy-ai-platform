...
async def run_sql_query(question: str, user: dict) -> dict:
    ...
    try:
        template_sql = await try_template_sql(question, user)
        if template_sql:
            generated_sql = template_sql
            guarded_sql = apply_scope(template_sql, user)
        else:
            ...
            guarded_sql = apply_scope(guarded_sql, user)
