...
2. **Model chính là Qwen2.5-3B** — mọi luồng AI phải tối ưu, benchmark và nghiệm thu với Qwen2.5-3B.
...
[Máy nền tảng]                     HTTP nội bộ              [Máy mô hình]
 web-ui, NestJS platform, ──► LLM / Embedding / Rerank ◄──  llm-server (Ollama, Qwen2.5-3B)
...
| **Ollama** | `llm-server` (Qwen2.5-3B, OpenAI-compatible API) |
| **Vite + React** | `web-ui` |
...
| `llm-server` | **Ollama** | Sinh text Qwen2.5-3B qua OpenAI-compatible API |
...
| **Máy mô hình**  | Build & serving model AI                        | `llm-server` (**Ollama**, Qwen2.5-3B), `embedding-server` (Python, BGE-M3), rerank (tùy chọn); GPU khuyến nghị |
...
- Kiến trúc 3 tầng (Client/Server); model chính = `Qwen2.5-3B`; frontend = Vite + React.
...
| LLM model (nghiệm thu) | **Qwen2.5-3B** — Máy mô hình riêng                              |
...
| RAG latency p95  | Theo baseline đo với Qwen2.5-3B, đủ dùng nội bộ     |
...
| ------------------------------- | ---------- | ---------------------------------------------------- |
| Qwen2.5-3B yếu ở câu phức tạp/SQL | Trung bình | Tăng grounding, rerank, view curation, eval liên tục |
