...
- **Chuẩn nghiệm thu (sau):** dev/test **2 máy** — **Máy nền tảng** (app + data) và **Máy mô hình** (build/serving Qwen2.5-3B); đổi `LLM_BASE_URL` / `EMBEDDING_BASE_URL` khi có server riêng.
- **Thành công:** traceability 100%, mọi acceptance gate pass với **Qwen2.5-3B** trên topology 2 máy.
...
Full parity · Model nghiệm thu **Qwen2.5-3B** · OpenAI-compatible API · On-premise first · Phân tầng · **Hiện tại: 1 máy Ubuntu** · Chuẩn sau: **2 máy** (Nền tảng + Mô hình) · Cấu hình tách code · Bảo mật mặc định · Tái lập được (Docker Compose).
...
- RAM ~16 GB — cần ưu tiên tiết kiệm tài nguyên nếu chạy đồng thời full stack Docker (5 kho DB + Milvus) và Ollama Qwen2.5-3B.
...
| Nghiệm thu | Tách Máy mô hình: profile `ai`, **Qwen2.5-3B** | Đổi URL trong `.env`; smoke cross-host |
...
4. Khi có server Máy mô hình: profile `ai` riêng, pull Qwen2.5-3B, cập nhật `.env`, chạy A-09 smoke cross-host.
...
- Model dev **`qwen2.5:3b`** (`LLM_MODEL` trong `.env`); đây cũng là model gate hiện tại cho M0–M4
...
- `llm-server` — Qwen2.5-3B, OpenAI-compatible API
...
- **M0 · chưa bắt buộc:** profile `ai`, Qwen2.5-3B, smoke cross-host — hoãn đến khi có Máy mô hình hoặc đủ RAM
...
- `docker compose --profile ai up` — **Qwen2.5-3B** + BGE-M3 (+ rerank) healthy
- Smoke cross-host: `LLM_BASE_URL` / `EMBEDDING_BASE_URL` trỏ IP Máy mô hình; latency baseline
- README bổ sung quickstart **2 máy**

**Deliverable**

- **#6 LLM Server** — `llm-server` **Ollama**, OpenAI-compatible API *(dev/hiện tại: `qwen2.5:3b`)*
...
- AI dev: **`qwen2.5:3b`** + localhost — đủ tích hợp `rag-engine` và là baseline hiện tại.

**Nghiệm thu dự án (topology 2 máy + Qwen2.5-3B)**
...
- Eval bắt buộc: RAG · SQL · Security/red-team · OCR/ingest · UAT theo module — với **Qwen2.5-3B**.
