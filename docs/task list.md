...
| AI      | D-06 · Hybrid chunking, overlap, giữ cấu trúc           | M2  | `[-]`   | `services/document-processor/app/chunker.py` đã chuyển sang **parent-child chunking**: detect heading `Phần/Chương/Điều/Mục`, dựng `parent_nodes` theo section và `child_nodes` overlap theo độ dài, kèm `section_path` / `child_ids` / `parent_preview`; **chưa** còn nhánh line-based/tabular split như v1 và chưa thấy regression test riêng cho luồng parent-child |
| AI      | D-07 · Prefix ngữ cảnh trước khi embed                  | M2  | `[-]`   | Luồng mới giữ ngữ cảnh chủ yếu qua `parent_id`, `section_path`, `parent_preview` và retrieval child→parent; **chưa** còn bước prefix tường minh `section_path` cho mọi chunk trước khi embed như `_with_section_prefix()` ở v1 |
