...
| Dữ liệu | C-06 · Seed dữ liệu nghiệp vụ mẫu và user đa role               | M0  | `[-]`   | IAM **partial done**: `generate_seed.py` + `12-seed-iam.sql` đã chuyển sang `pbkdf2_sha256` + `password_salt` + `hash_iterations=100000`, đồng bộ lại các seed `11–14`; `seed-iam.ps1`; fresh DB seed end-to-end re-verify 2026-06-24 trên disposable Postgres: `users=74`, `roles=6`, `permissions=16`, `hoc_vien=50`, `exam_banks=15`, `user_sessions` table tồn tại; ví dụ login thật `admin/BGD/p2_01/676156`; **chưa** validate full re-run generator |
...
| AI      | D-06 · Hybrid chunking, overlap, giữ cấu trúc           | M2  | `[x]`   | `services/document-processor/app/chunker.py` nay tách section ổn định theo `Phần/Chương/Điều/Mục`, giữ `section_path` không dính sibling section, dựng `parent_nodes` + `child_nodes` kèm `child_ids` / `parent_preview`, đồng thời giữ lại wrapper `chunk_document` / `chunk_text` để không mất nhánh plain-text/tabular v1; có regression `services/document-processor/tests/test_chunker.py` cho parent-child path và re-run pass 2026-06-24 |
...
| AI        | UC-AI-01..05 — GenAI/RAG         | M2–M4    | `[-]`   | Phase 1 **done:** chat LLM local (`qwen2.5:3b`) + history (**G-12** `[x]`, **K-12** `[x]`); Phase 2 **gần xong:** ingest/OCR/chunk/embed (**D-01..D-12** `[x]`, gồm parent-child chunking/prefix/re-ingest cleanup), RAG retrieve/rerank/grounding (**E-01**, **E-02**, **E-05**, **E-08** `[x]`; **E-03..E-04** partial), multi-turn Redis E2E (**E-06** `[x]`), safe refusal (**E-07** `[x]`), eval harness (**E-09** `[x]`); Text-to-SQL read-only + format UX (**F-01..F-08** `[x]`); **còn:** đồng bộ row-filter sâu hơn giữa RAG/Postgres và tuning rerank/context |
| Quản trị  | UC-QT-01..04 — quản trị hệ thống | M0/M5/M6 | `[-]`   | JWT login + refresh cookie (**K-02** `[x]`, **G-06** `[x]`); admin health live + AI policy editor (**K-08** `[x]`, **K-09** `[x]`); admin quota/token/account ops (**K-10** `[x]`); audit UI/detail/export (**K-07** `[x]`); RBAC/audit backend (**G-02..G-05** `[x]`); browser automation smoke (**K-11** `[x]`); **còn:** penetration/rate-limit (**G-07**, **G-11**) |
...
## Ưu tiên tiếp theo (cập nhật 2026-06-24 — M0 bootstrap, parent-child retrieval, K-07 và K-10 admin flows đã re-verify/xong)
...
**Trạng thái chat hiện tại** — Nền chat/RAG + SQL read-only + policy/RBAC an toàn đã nối end-to-end qua gateway; nhánh **parent-child chunking / retrieval** đã có đường section-path ổn định, prefix ngữ cảnh trước embed, cleanup re-ingest, router `reject/task_assist` và Milvus metadata push-down theo `document_id`, đồng thời đã re-run targeted tests pass ngày 2026-06-24. Bootstrap M0 cũng đã verify lại `up-code.ps1`, data-service connectivity và fresh DB seed; nhánh quản trị nay đã khép thêm **K-07 + K-10** (audit viewer/export, quota/token/account ops + account status/session actions). Phần còn thiếu rõ nhất: **K-04** phase 2 (timeline/chunk preview), **E-03/E-04** (scope alignment sâu hơn + rerank/context tuning), **G-07/G-11** (hardening/rate-limit).
...
| 19 | **G-07 / G-11** | Rate limit, penetration hardening |
